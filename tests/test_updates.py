...
class UpdateCheckerTests(unittest.TestCase):
    def test_checker_uses_public_versioned_api_and_reports_update(self) -> None:
        ...
        self.assertEqual(options["headers"]["X-GitHub-Api-Version"], "2022-11-28")
...
class UpdateDownloaderTests(unittest.TestCase):
    def test_download_verifies_hash_and_saves_atomically(self) -> None:
        installer = b"MZ installer payload"
    ...
    def test_checksum_mismatch_blocks_and_removes_partial_file(self) -> None:
    ...
    def test_cancelled_download_never_publishes_an_installer(self) -> None:
        update, session = _download_fixture(b"installer")
        results = queue.Queue()
        with tempfile.TemporaryDirectory() as directory:
            downloader = UpdateDownloader(
