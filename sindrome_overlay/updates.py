...
class UpdateDownloader(threading.Thread):
    """Download and authenticate one installer without blocking Qt."""
    ...
    def __init__(
        ...
        *,
        ...
        session: requests.Session | None = None,
        signature_verifier: Callable[[Path, str], AuthenticodeResult] = verify_authenticode,
        ...
        if current_executable is None and getattr(sys, "frozen", False):
    ...
    def run(self) -> None:
        try:
            ...
            expected_signer = self._current_signer_subject()
            ...
            signature = self.signature_verifier(partial_path, expected_signer)
            if not signature.valid:
                code = "signer_mismatch" if signature.error_code == "signer_mismatch" else "signature"
                raise _UpdateFailure(code, signature.status)
    ...
    def _current_signer_subject(self) -> str:
        if self.current_executable is None:
            return ""
        signature = self.signature_verifier(self.current_executable, "")
        if not signature.valid:
            raise _UpdateFailure("current_signature", signature.status)
        return signature.signer_subject
